...
//
    ...
    override func viewDidLoad() {
        ...
        self.mPTTextField.inputAccessoryView = self.accessory
        self.addAccessory()
        self.mPTTextField.fontColor = .blue
        self.mPTTextField.clearBtnColor = .red
        self.mPTTextField.textShow = "นามแฝง/อีเมล"
        self.mPTTextField.borderColor = .blue
        self.mPTTextField.setupPTTextField()
...
//        accessoryView.addBorder(edges: .top, colour: UIColor.purple, thickness: 0.3)
    ...
    func addAccessory()  {
        self.accessory.frame = CGRect(x: 0, y: 0, width: 300, height: 48)
...
//        mButton.addTarget(self.superview?.viewController, action: #selector(self.superview?.viewController?.donePressed), for: .touchUpInside)
        mButton.addTarget(self, action: #selector(self.endEditing), for: .touchUpInside)
        ...
        mButton.trailingAnchor.constraint(equalTo: self.accessory.trailingAnchor, constant: -8).isActive = true
    }
    
    @objc func endEditing() {
        self.mPTTextField.endEditing(true)
    }
...
//        print("target1: ",self)
////        self.oPTTextField.delegate = self
//        self.oPTTextField.setupPTTextField(target: self, sTextShow: "Hello World")
////        self.oPTTextField.addTarget(self, action: #selector, for: .editingChanged)
